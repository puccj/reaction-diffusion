...
Mesh::Mesh(int i, int j, int k, double data) :
              _i{i}, 
              _j{j},
              _k{k},
...
double Mesh::der2(int i, int j, int k) {
  ...
  return (back + front + down + up + right + left - 6*_data[i*_j*_k + j*_k + k]) /(_h*_h);
}

Mesh Mesh::der2() {
  Mesh derivative(_i, _j, _k);
...
Mesh operator*(Mesh lhs, const double& rhs) {
  lhs *= rhs;
  return lhs;
}

Mesh operator*(const double& lhs, Mesh rhs) {
  rhs *= lhs;
  return rhs;
}
*/
...
{
  //format output: 4 cols data:  x y z values
